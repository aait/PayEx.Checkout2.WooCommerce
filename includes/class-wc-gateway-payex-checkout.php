...
class WC_Gateway_Payex_Checkout_V2 extends WC_Gateway_Payex_Cc
	...
	public function payment_scripts() {
		...
		if ( is_checkout() || isset( $_GET['pay_for_order'] ) || is_add_payment_method_page() ) {
			wp_register_script( 'wc-gateway-payex-checkout-v2', untrailingslashit( plugins_url( '/', __FILE__ ) ) . '/../assets/js/checkout.js', array(
				'jquery',
				'wc-checkout',
				'featherlight'
			), FALSE, TRUE );
			...
			wp_localize_script( 'wc-gateway-payex-checkout-v2', 'WC_Gateway_PayEx_Checkout_v2', $translation_array );

			// Enqueued script with localized data.
			wp_enqueue_script( 'wc-gateway-payex-checkout-v2' );

			// Add OnePage Script
			wp_enqueue_script( 'wc-payex-onepage', untrailingslashit( plugins_url( '/', __FILE__ ) ) . '/../assets/js/onepage.js', array(
				'jquery', 'wc-gateway-payex-checkout-v2',
			), FALSE, FALSE );
		}
	}
...
// Register Gateway
WC_Payex_Psp::register_gateway( 'WC_Gateway_Payex_Checkout_V2' );
